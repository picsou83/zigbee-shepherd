...
af.send = function (srcEp, dstEp, cId, rawPayload, opt, callback) {
    ...
    areq.register(afEventCnf, deferred, function (cnf) {
        ...
        if (cnf.status === 0 || cnf.status === 'SUCCESS')   // success
            ...
            var test = dstEp.getIeeeAddr();
                var dev = shepherd._findDevByAddr(test);
                  if (dev.status != 205) {
                    console.log('Attention status 205 pour ' + test);
					dev.update({ status: '205' });
                    af.controller.request('ZDO', 'nwkAddrReq', { ieeeaddr: test, reqtype: 0, startindex:1 });
               		shepherd._devbox.sync(dev._getId(), function () {});
				}}
