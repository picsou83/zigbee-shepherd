...
function ZShepherd(path, opts) {
    ...
    this.on('ind:statusChange', function (ep, cId, payload, msg) {
        ...
        self.emit('ind', { type: 'statusChange', endpoints: [ ep ], data: notifData, linkquality: msg.linkquality, groupid: msg.groupid });
    });
    ...
    return Q.fcall(function () {
        ...
        if (rsp.status === 0)  // success
            return _.map(rsp.neighborlqilist, function (neighbor) {
                return { ieeeAddr: neighbor.extAddr, nwkAddr: neighbor.nwkAddr, lqi: neighbor.lqi, depth: neighbor.depth, deviceType: neighbor.deviceType };
            });
    }).nodeify(callback);
};
...
ZShepherd.prototype.lqiScan = function (ieeeAddr) {
            ...
            data.forEach(function (devinfo) {
                ...
                if (dev && dev.type == "Router" && !noDuplicate[ieeeAddr]) {
                    chain = chain.then(function () {
                        return self.lqi(ieeeAddr).timeout(3000).then(processResponse(ieeeAddr));
                    });
                }
    ...
    return self.lqi(ieeeAddr)
        ...
        .then(function(){
            return objectValues(noDuplicate);
        })
        .catch(function(){
            return objectValues(noDuplicate);
        });
